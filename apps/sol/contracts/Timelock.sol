...
contract Timelock {
    ...
    uint public constant MINIMUM_DELAY = 2 minutes; // For Demo
    ...
    function setPendingAdmin(address pendingAdmin_) public {
        ...
        require(msg.sender == admin, "Timelock::setPendingAdmin: Call must come from Timelock."); // for demo
        pendingAdmin = pendingAdmin_;

        emit NewPendingAdmin(pendingAdmin);
    }

    function queueTransaction(address target_, uint value_, string memory signature_, bytes memory data_, uint eta_) public returns (bytes32) {
        require(msg.sender == admin, "Timelock::queueTransaction: Call must come from admin.");
        require(eta_ >= getBlockTimestamp() + delay, "Timelock::queueTransaction: Estimated execution block must satisfy delay.");
