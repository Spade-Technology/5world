...
const BlogCard = ({ blog, showBorder, id }: cardProps) => {
  return (
    <div
      className={` grid-cols-12 gap-12 md:grid md:pb-16  ${showBorder ? 'border-b-[1px] border-black ' : ''} ${
        id > 0 ? 'pt-10' : ''
      } `}
    >
      ...
      <div className='w-full pt-[34px] md:col-span-8 md:pt-0 lg:col-span-6'>
        ...
        <div className='pt-5 font-heading text-[26px] font-medium leading-[30px] md:text-3xl md:leading-9'>
          {blog.heading}
        </div>
        ...
        <div className='mt-[18.8px] flex flex-wrap gap-2.5'>
          {blog.features &&
            ...
            blog.features.map((item: string, idx: number) => {
              return (
                <div
                  className='w-fit cursor-pointer rounded-[20px] bg-vdao-purple px-6 py-1 font-body text-sm font-normal text-black md:rounded-3xl'
                  key={idx}
                >
                  {item}
                </div>
